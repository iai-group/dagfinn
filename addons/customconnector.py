...
class SocketBlueprint(Blueprint):
    def __init__(
        ...
        Args:
            sio: Instance of :class: class `socketio.AsyncServer`
            socketio_path: Indicating the route to accept requests on.
        """
...
class SocketIOOutput(OutputChannel):
    ...
    def __init__(self, sio: AsyncServer, bot_message_evt: Text) -> None:
        """Creates a ``Asyncserver`` object for emitting events."""
    ...
    async def _send_message(self, socket_id: Text, response: Any) -> None:
        ...
        await self.sio.emit(self.bot_message_evt, response)
        # removed last parameter room=socket_id resulting in a broadcast
        # if no room is provided
    ...
    async def send_text_with_buttons(
        ...
        **kwargs: Any,
        ...
        # the `or` makes sure there is at least one message
        # we can attach the quickreplies to
    ...
    async def send_custom_json(
        ...
        # commented to make sure the event is broadcasted
...
class Custom_socketIOInput(InputChannel):
    ...
    def from_credentials(
        ...
        return cls(
            ...
            credentials.get("socketio_path", "/furhat"),
    ...
    def __init__(
        ...
        socketio_path: Optional[Text] = "/furhat",
        jwt_key: Optional[Text] = None,
        jwt_method: Optional[Text] = "HS256",
    ):
        """Creates a ``Custom_socketIOInput`` object."""
    ...
    def blueprint(  # noqa  Escaping: C901 (too complex)
        ...
        custom_socketio_webhook = SocketBlueprint(
        ...
        @custom_socketio_webhook.route("/", methods=["GET"])
        ...
        async def connect(
            ...
            if self.jwt_key:
                ...
                if auth and auth.get("token"):
                    jwtPayload = rasa.core.channels.channel.decode_bearer_token(
        ...
        @sio.on("reset_story")
        async def handle_reset(sid: Text) -> None:
            output_channel = SocketIOOutput(sio, self.bot_message_evt)
            message = UserMessage(
                "/restart", output_channel, sid, input_channel=self.name()
            )
            await on_new_message(message)
            await self.sio.emit("reset_frontend")

        @sio.on(self.user_message_evt, namespace=self.namespace)
        async def handle_message(sid: Text, data: Dict) -> None:
            output_channel = SocketIOOutput(sio, self.bot_message_evt)

            broadcastUsrMsg = SocketIOOutput(sio, self.user_message_evt)
            ...
            await broadcastUsrMsg.send_text_message(sender_id, data["message"])
            await on_new_message(message)

        return custom_socketio_webhook
